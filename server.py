...
Правила:
0. Язык курса строго "{language}". Не переключайся на другой язык из-за отдельных слов,
   примеров, оглавления или ошибок OCR. Все грамматические темы и примеры должны относиться
   только к языку "{language}". В detectedLanguage верни именно "{language}".
